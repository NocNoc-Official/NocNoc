...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::NOC);
    ...
    if (!settings.contains("nAnonymizeNocNocAmount"))
        settings.setValue("nAnonymizeNocNocAmount", 1000);

    nAnonymizeNocNocAmount = settings.value("nAnonymizeNocNocAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeNocNocAmount"))
        SoftSetArg("-anonymizeNocNocamount", settings.value("nAnonymizeNocNocAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in NocNoc.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeNocNocAmount:
            return QVariant(nAnonymizeNocNocAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeNocNocAmount:
            nAnonymizeNocNocAmount = value.toInt();
            settings.setValue("nAnonymizeNocNocAmount", nAnonymizeNocNocAmount);
            emit anonymizeNocNocAmountChanged(nAnonymizeNocNocAmount);
            break;
        case CoinControlFeatures:
