...
void PrivacyDialog::on_pushButtonMintzICH_clicked()
{
    ...
    if (encStatus == walletModel->Locked) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Mint_zNOC, true));
        if (!ctx.isValid()) {
...
void PrivacyDialog::on_pushButtonSpendzICH_clicked()
{
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Send_zNOC, true));
        if (!ctx.isValid()) {
            // Unlock wallet was cancelled
            return;
        }
...
void PrivacyDialog::sendzICH()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid NocNoc Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " NOC </b>will be added to the standard transaction fees!<br />";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " NOC, ";
