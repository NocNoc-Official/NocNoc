...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("NocNocMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("NocNocMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("NocNocMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("NocNoc-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running NocNocMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
